...
export default function FertilizersPageConfig() {
  const pageName = 'fertilizers';
  const pageTitle = 'Fertilizers';
  
  const { data: config, isLoading } = usePageConfig(pageName);
  const updateConfig = useUpdatePageConfig();
  const { data: allProducts } = useProducts({ categoryType: 'FERTILIZERS', limit: 100 });
  ...
  const fertilizerCategories = React.useMemo(() => {
    if (!allCategories) return [];
    return allCategories.filter((c: any) => c.type === 'FERTILIZERS');
  }, [allCategories]);
  
  const fertilizerProducts = React.useMemo(() => {
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        {/* Fertilizer Categories Section */}
        <Card className="mt-8">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle>Fertilizer Categories</CardTitle>
              <CardDescription>Manage fertilizer categories</CardDescription>
            </div>
            <Dialog open={categoryDialogOpen} onOpenChange={setCategoryDialogOpen}>
              ...
              <DialogContent>
                ...
                <div className="space-y-3 py-3">
                  ...
                  <Textarea placeholder="Description" value={categoryForm.description} onChange={(e) => setCategoryForm(v => ({ ...v, description: e.target.value }))} />
                  <Button className="w-full" onClick={async () => {
                    try {
                      if (editingCategory) {
                        await updateCategory.mutateAsync({ id: editingCategory.id, data: { ...categoryForm, type: 'FERTILIZERS' } });
                      } else {
                        await createCategory.mutateAsync({ ...categoryForm, type: 'FERTILIZERS' });
                      }
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {fertilizerCategories.map((cat: any) => (
        ...
        <Card className="mt-8">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>Manage fertilizer products in this category</CardDescription>
            </div>
            <Link href="/admin/products/new?category=FERTILIZERS">
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {fertilizerProducts.slice(0, 12).map((product) => (
                  <Card key={product.id} className="overflow-hidden">
                    ...
                    <CardContent className="p-3">
                      ...
                      <p className="text-amber-600 font-bold text-sm">{product.price ? `৳${product.price.toLocaleString()}/${product.priceUnit}` : 'Price on request'}</p>
        ...
        <Card className="mt-8">
          <CardHeader>
            <CardTitle>All Fertilizer Products</CardTitle>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                {fertilizerProducts.map((product: any) => (
